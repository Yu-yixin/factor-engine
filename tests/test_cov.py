...
def test_engine_evaluate_cov():
    ...
    result = FactorEngine().evaluate("cov(close, volume, 3)", df)
    values = result["result"].to_list()
    expected = [None, None, 1.0, -10.0, 2.0, -20.0, 2.0, -20.0]
...
def test_cov_requires_window_at_least_two():
    df = pl.DataFrame(
        {
            "time": [1, 2],
            "code": ["A", "A"],
    ...
    with pytest.raises(ExecutionError, match="window >= 2"):
        FactorEngine().evaluate("cov(close, volume, 1)", df)


def test_engine_evaluate_cov_with_cross_sectional_inputs_matches_split_steps():
    df = pl.DataFrame(
        {
    ...
    split = engine.evaluate("cov(rank_open, rank_volume, 3)", step)
    nested = engine.evaluate("cov(rank(open), rank(volume), 3)", df)
