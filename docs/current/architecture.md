...
- `execution_cse.py`: shared DAG-node materialization shell, CSE child read accounting, and materialized helper byte estimation.
- `execution_lifecycle.py`: executor integration helpers for lifecycle trace events, helper-drop revalidation, nested-drop ordering checks, and lifecycle step-model assertions.
